...
const DEFAULT = { speed: 2.5, gainDb: -4, maxDur: 400 };
...
document.querySelectorAll('[data-preset-speed]').forEach(function (btn) {
  btn.addEventListener('click', function () {
    var v = parseFloat(btn.dataset.presetSpeed);
    slSpeed.value = v;
...
function playPreview() {
  ...
  isPreviewPlaying = true; updatePreviewBtn(true);
...
function drawWaveform(buffer) {
  ...
  var mid = H / 2;
  ctx.beginPath(); ctx.strokeStyle = '#6366f1'; ctx.lineWidth = 1;
  for (var x = 0; x < W; x++) {
    var mn = 1, mx = -1;
    for (var j = 0; j < step; j++) { var v = data[x * step + j] || 0; if (v < mn) mn = v; if (v > mx) mx = v; }
    ctx.moveTo(x, mid + mn * mid * 0.85); ctx.lineTo(x, mid + mx * mid * 0.85);
  }
  ctx.stroke();
}
...
function renderResult(r) {
  ...
  dlNote.innerHTML =
    ...
    'Kecepatan ' + r.speed + '× · Amplifikasi ' + r.gainDb + ' dB · Maks ' + r.maxDur + 's<br>' +
    'Speed Normal di Roblox: <strong>' + robloxSpeed + '</strong><br>' +
    'Codec: OGG Vorbis (Roblox-compatible)' +
    (warnings.length ? '<br><span style="color:#b45309">' + warnings.join('<br>') + '</span>' : '');

  dlSize.textContent = 'Ukuran — OGG: ' + formatBytes(r.oggBlob.size) + ' · MP3: ' + formatBytes(r.mp3Blob.size);
